...
test('left click moves the one spell between book and preparation, right click opens a menu, slots stay off this page',async({page})=>{
 ...
 await page.getByRole('switch',{name:'编辑模式'}).click();await first.hover();await expect(page.locator('.entry-detail')).toContainText('星光矢');await first.click();await expect(page.locator('.prepared-cell .spell-stock-tile')).toContainText('星光矢');await expect(page.locator('.spell-stock-tile').filter({hasText:'星光矢'})).toHaveCount(1);
...
test('daily full-list caster gets the eligible source list and can prepare without duplicating all spells into the card',async({page})=>{
 ...
 await page.getByRole('switch',{name:'编辑模式'}).click();await expect(page.locator('.spell-library')).toContainText('职业法术表');await expect(page.locator('.spell-library .spell-stock-tile')).toHaveCount(2);await page.locator('.spell-library .spell-stock-tile').filter({hasText:'一环祈祷'}).click();await expect(page.locator('.prepared-cell .spell-stock-tile')).toHaveCount(1);await expect(page.locator('.spell-library .spell-stock-tile')).toHaveCount(1);await page.locator('.prepared-cell .spell-stock-tile').click();await expect(page.locator('.spell-library .spell-stock-tile')).toHaveCount(2);
});
...
test('spell tiles double their height and retain a right-aligned tier through preparation and narrow layout',async({page})=>{
 ...
 await page.getByRole('switch',{name:'编辑模式'}).click();await tiles.filter({hasText:'三环星尘'}).click();const prepared=page.locator('.prepared-cell .spell-stock-tile');await expect(prepared.locator('.spell-stock-level')).toHaveText('3');await expect(page.locator('.spell-tile-flight')).toHaveCount(0);await page.mouse.move(2,2);await page.screenshot({path:'F:/CodexWork/2026-09-20/w-xu/spells181-tier-wide.png'});
