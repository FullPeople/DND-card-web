...
test('catalog auto-appends near bottom; glossary excludes unrelated data and conditions default to handbooks',async({page})=>{
 ...
 await page.goto('/');await expect(page.getByRole('button',{name:'更新资料',exact:true})).toBeEnabled();const tabs=page.getByRole('navigation',{name:'资料分类'});await tabs.getByRole('button',{name:'法术',exact:true}).click();await expect(page.locator('.catalog-row')).toHaveCount(80);await page.locator('.catalog-list').evaluate(e=>{e.scrollTop=e.scrollHeight;});await expect(page.locator('.catalog-row')).toHaveCount(160);await page.locator('.catalog-list').evaluate(e=>{e.scrollTop=e.scrollHeight;});await expect(page.locator('.catalog-row')).toHaveCount(190);
 ...
 await tabs.getByRole('button',{name:'状态',exact:true}).click();await expect(page.locator('.catalog-row')).toHaveCount(1);await expect(page.locator('.catalog-row')).toContainText('核心状态');await page.getByRole('button',{name:'清空',exact:true}).click();await expect(page.locator('.catalog-row')).toHaveCount(2);await page.reload();await expect(page.locator('.catalog-row')).toHaveCount(2);
});
