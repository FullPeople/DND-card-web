...
// Five-page physics and 18 concurrent effects need a larger end-to-end budget.
...
test('highest class watermark follows levels, tie order, removal, page changes and refresh',async({page})=>{
 ...
 await openCard(page,c);const badge=page.locator('.class-watermarks img');await expect(badge).toHaveCount(1);await expect(badge).toHaveAttribute('data-class-badge','wizard');
 await page.getByRole('spinbutton',{name:'战士等级',exact:true}).fill('2');await expect(badge).toHaveAttribute('data-class-badge','wizard');
 await page.getByRole('spinbutton',{name:'战士等级',exact:true}).fill('3');await expect(badge).toHaveAttribute('data-class-badge','fighter');
 ...
 await page.reload();await expect(badge).toHaveAttribute('data-class-badge','fighter');await page.getByRole('tab',{name:/主要/}).click();
...
test('all effects coexist, death saves persist, editing restores five pages and physical hit boxes follow gravity',async({page})=>{
 ...
 await expect(page.locator('.status-strip .feature-bubble')).toHaveCount(18);
 ...
 await page.getByRole('spinbutton',{name:'当前生命值',exact:true}).fill('7');await expect(page.getByRole('spinbutton',{name:'当前生命值',exact:true})).toHaveValue('7');
 await page.getByRole('spinbutton',{name:'力竭层数',exact:true}).fill('4');await expect(page.locator('.death-saves-cell .exhaustion-segment.segment-on')).toHaveCount(4);
 ...
 for(const name of ['特性','背景','法术','背包']){
   await page.getByRole('tab',{name:new RegExp(name)}).click();await page.waitForTimeout(1100);await noOverlap(page);await expect(page.locator('.status-strip .feature-bubble')).toHaveCount(18);await expect(page.locator('.portrait-cell')).toHaveCSS('rotate','180deg');
...
test('inherited visuals survive removal of one parent, duplicate sources deduplicate and fear keeps stationary hit boxes',async({page})=>{
 ...
 const first=await page.getByRole('combobox',{name:'当前角色',exact:true}).locator('option').first().getAttribute('value');await page.getByRole('combobox',{name:'当前角色',exact:true}).selectOption(first!);await expect(page.locator('[data-card-effect]')).toHaveCount(0);
});
