...
test('concentration sigil stays beside the tier and both click and drag smoothly close the departing spell gap',async({page})=>{
 ...
 await focus.click();await expect(page.locator('.prepared-cell .spell-stock-tile')).toHaveCount(1);expect(await page.evaluate(()=>(window as any).reflows.filter((v:any)=>v.id).length)).toBeGreaterThanOrEqual(6);
