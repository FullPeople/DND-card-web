...
import {inferredSpellMode} from './spellcastingRules';
...
export function spellState(c:Character):SpellSettings{
 const ability=c.selections.find(s=>s.entry.kind==='class'&&ABILITIES.includes(s.entry.raw.spellcastingAbility))?.entry.raw.spellcastingAbility||'int';
 ...
 return {...stored,capacityAdjustment:adjustment,capacity:Math.max(0,Math.min(100,(base||0)+adjustment)),mode:stored.modeOverride?stored.mode:inferredSpellMode(c)||stored.mode};
}
