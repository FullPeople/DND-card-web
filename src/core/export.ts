import { ABILITIES, ABILITY_LABELS, KIND_LABELS, SKILLS, selectionAllowed, signed, type Character, type Derived, type RulePack } from './model';
...
export function exportOwlbear(c: Character, d: Derived) {
  ...
  return { schema_version: '0.3', meta: { template_name: 'DND Card Web', template_version: '0.1.0', layout_version: 'web-1', ruleset: c.edition, source_file: `${c.name}.json`, parsed_at: new Date().toISOString() },
    identity: { character_name: c.name, display_name: c.name, player: c.player, race: { name: entryName('race'), subrace: null }, background: entryName('background'), alignment: c.identity.alignment || null, gender: c.identity.gender || null, age: c.identity.age || null, languages: [], tool_proficiencies: [] },
    ...
    skills: Object.entries(d.skills).map(([key, s]) => ({ name: SKILLS[key].name, ability: SKILLS[key].ability, total: s.value, proficiency: s.proficient ? 'proficient' : 'none', misc_bonus: s.value - d.modifiers[SKILLS[key].ability] - (s.proficient ? d.proficiency : 0) })),
    ...
    export_warnings: ['填写要求、选择历史和自定义规则包请保留在原生角色备份中。法术暂放入已准备列表；法术位、武器攻击、抗性和复杂特性需在枭熊中核对。', ...d.requirements.filter(r => !r.complete).map(r => `未完成：${r.label}（${r.origin}）`), ...d.issues.map(i => i.message), ...(c.adjustments || []).map(a => `人工修正 ${a.target}=${a.value}：${a.reason}`)] };
}
...
export function exportReview(c: Character, d: Derived): string {
  ...
  const sections = [
    ...
    `<p>等级 ${d.level} · AC ${d.ac} · HP ${c.runtime.hp}/${d.maxHp}（临时 ${c.runtime.tempHp}）· 先攻 ${signed(d.initiative)} · 速度 ${d.speed} 尺 · 熟练 ${signed(d.proficiency)} · 被动察觉 ${d.passive} · 生命骰 ${esc(d.hitDice)}</p>`,
    ...
    '<h2>填写与核对</h2>', `<ul>${pending.map(r => `<li>${esc(r.label)} — ${esc(r.origin)}（${r.selected.length}/${r.count}）</li>`).join('') || '<li>已完成当前支持的填写检查；特殊规则仍以 DM 裁定为准。</li>'}${d.issues.map(i => `<li>${esc(i.message)}</li>`).join('')}</ul>`,
    '<h2>已经记录的选择</h2>', table(['选择要求', '已选内容'], Object.entries(c.answers).map(([id, values]) => { const r = d.requirements.find(r => r.id === id); return [r ? `${r.origin} · ${r.label}` : `历史选择（原要求已失效）：${id}`, values.map(v => r?.optionLabels?.[v] || choiceLabel(v)).join('、')]; })),
    `<h2>启用规则</h2><p>${c.profile.enabledSources.map(esc).join('、')}</p><p>专长：${c.profile.optional.feats ? '开' : '关'}；兼职：${c.profile.optional.multiclass ? '开' : '关'}；旧版兼容：${c.profile.optional.legacy ? '开' : '关'}</p>`,
    '<h2>人工裁定与数值修正</h2>', `<ul>${Object.entries(c.profile.exceptions).map(([id, reason]) => `<li>条目特许 · ${esc(c.selections.find(s => s.entry.id === id)?.entry.name || id)}：${esc(reason)}</li>`).join('')}${(c.adjustments || []).map(a => `<li>${esc(a.target)} → ${a.value}：${esc(a.reason)}</li>`).join('') || '<li>无</li>'}</ul>`,
    '<h2>当前资源</h2>', table(['资源', '剩余', '上限'], Object.entries(c.runtime.resources).map(([name, v]) => [name, v.current, v.max])),
    '<h2>角色条目与固定来源</h2>', ...c.selections.map(s => `<article><h3>${esc(s.entry.name)} <small>${esc(KIND_LABELS[s.entry.kind])}</small></h3><p class="muted">${esc(s.entry.source)} · ${esc(s.entry.edition)} · 资料修订 ${esc(s.entry.revision)}${s.entry.page ? ` · 第 ${s.entry.page} 页` : ''}</p><p>${s.entry.kind === 'class' ? `${s.level} 级 · ` : ''}${s.entry.kind === 'item' ? `数量 ${s.quantity} · ${s.equipped ? '已装备' : '未装备'} · ` : ''}${selectionAllowed(c, s.entry) ? '来源已启用' : '来源或规则未启用，效果暂停'}${s.requirementId ? ` · 对应要求：${esc(d.requirements.find(r => r.id === s.requirementId)?.label || '原要求已失效')}` : ''}${c.reviewed.includes(s.id) ? ' · 特殊效果已人工核对' : ''}</p><pre>${esc(plainText(s.entry.entries))}</pre></article>`),
