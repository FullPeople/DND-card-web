...
export function SpellSlotResources({rows,render}:{rows:ResourceValue[];render:(row:ResourceValue)=>ReactNode}){
 ...
 return <><button ref={trigger} className="spell-slot-summary" aria-expanded={!!point} onClick={e=>{const r=e.currentTarget.getBoundingClientRect();setPoint(point?undefined:{x:e.detail?e.clientX:r.left,y:e.detail?e.clientY:r.bottom});}}><span>法术位</span><span>{slots.map(r=>r.current).join('/')}</span></button>{point&&createPortal(<div ref={panel} role="dialog" aria-label="法术位" className="spell-slot-popover" style={{left:point.x,top:point.y}}><strong>法术位</strong><div className="resource179-flow">{slots.map(r=><div key={r.id}>{render(r)}</div>)}</div></div>,document.body)}</>;
}
