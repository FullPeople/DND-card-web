...
export function Portrait({c,edit,field='portrait'}:{c:Character;edit:Edit;field?:'portrait'|'illustration'}){
 const target=useWorkbench().target,tokenUrl=field==='portrait'&&target?.kind==='character'&&workbenchCharacterId(target)===c.id?target.tokenPortrait?.url:undefined;
 const fallback=useMemo<Character['portrait']>(()=>tokenUrl?{data:tokenUrl,x:0,y:0,zoom:1}:undefined,[tokenUrl]);
 const label=field==='illustration'?'立绘':'头像',saved=c[field]||fallback,editing=useContext(SheetEditContext),view=useRef<HTMLDivElement>(null),[moving,setMoving]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(''),[deleteConfirm,setDeleteConfirm]=useState(false);
 ...
 const frame=(image:NonNullable<Character['portrait']>)=>{const el=view.current!,p=position(image);return {...image,x:p.x*el.clientWidth/100,y:p.y*el.clientHeight/100,frameWidth:el.clientWidth,frameHeight:el.clientHeight};};
 useEffect(()=>{const el=view.current;if(!el||!editing)return;const wheel=(e:WheelEvent)=>{if(!latest.current||!(e.target as Element).closest('.portrait-move'))return;e.preventDefault();e.stopPropagation();const next={...frame(latest.current),zoom:Math.max(1,Math.min(5,latest.current.zoom*Math.exp(-e.deltaY*.0015)))};setDraft(next);edit(c=>{c[field]=next;},`${field}-zoom`);};el.addEventListener('wheel',wheel,{passive:false});return()=>el.removeEventListener('wheel',wheel);},[editing,edit,field]);
