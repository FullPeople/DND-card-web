...
export function SourceSettings({c,entries,edit,readOnly=false,summary=false,changeMode}:{c:Character;entries:Entry[];edit:(action:(draft:Character)=>void,mode?:SourceMode)=>void;readOnly?:boolean;summary?:boolean;changeMode?:(mode:SourceMode)=>void}){
 ...
 <div className="dialog-actions"><button disabled={readOnly} onClick={()=>edit(d=>{d.profile.enabledSources=['PHB','XPHB'];})}>仅基础规则</button><button disabled={readOnly} onClick={()=>edit(d=>{d.profile.enabledSources=sources;})}>全部开启</button><button disabled={readOnly} onClick={()=>edit(d=>{d.profile.enabledSources=[];})}>全部禁用</button></div>
