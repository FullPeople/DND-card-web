...
type Props = { inline?: boolean; grouped?: boolean; c: Character; rows: Selection[]; receive?: (entry: Entry) => void; label?: string; className?: string; kinds?: Kind[]; edit: (action: (draft: Character) => void) => void; browse: () => void; onLink: (reference: string, kind?: string) => void; children?: ReactNode };

export function FeaturePanel({ inline = false, c, rows, receive, edit, browse, onLink, children, label = '特性', className = 'class-features', kinds = ['feature', 'rule'], grouped = true }: Props) {
  const {format}=useSources();
  const spells = kinds.length === 1 && kinds[0] === 'spell';
  const features: Feature[] = rows.map(s => {
    const owner = c.selections.find(p => p.id === s.parentId || s.requirementId?.startsWith(`${p.id}:`)) || c.selections.find(p => p.entry.kind === 'class' && [p.entry.name, p.entry.english].includes(s.entry.raw.className) && (!s.entry.raw.classSource || p.entry.source === s.entry.raw.classSource));
    return { id: s.id, group: spells ? String(s.entry.raw.level ?? 0) : grouped ? owner?.id || 'other' : 'all', origin: owner ? `${KIND_LABELS[owner.entry.kind]} ${owner.entry.name}${['class', 'subclass'].includes(owner.entry.kind) ? ` Lv.${owner.level}` : ''}` : '其他特性', name: s.entry.name, entry: s.entry, body: s.entry.entries, restricted: !selectionAllowed(c, s.entry) || !!owner && !selectionAllowed(c, owner.entry) };
  });
  ...
  const expanded = new Set(inline ? [] : saved?.expanded || []);
  ...
  function saveLayout(order: string[], open: string[]) {
    live.current.edit(draft => {
      ...
      draft.featureLayout = { order: [...(draft.featureLayout?.order || []).filter(id => !own.has(id)), ...order], expanded: [...(draft.featureLayout?.expanded || []).filter(id => !own.has(id)), ...open] };
    });
  }
  ...
  const contents = <>
    ...
    {groups.map(group => <section className={`feature-group ${spells ? 'spell-level-group' : ''}`} data-spell-level={spells ? group.group : undefined} style={spells ? { '--spell-shade': `${6 + Math.min(9, Number(group.group)) * 6}%` } as CSSProperties : undefined} key={group.items[0].id}>{spells && <span className="spell-level-number" aria-label={`${group.group}环`}>{group.group}</span>}{grouped && <h4>{group.origin}</h4>}<div className="feature-flow">{group.items.map(f => <article key={f.id} data-feature-id={f.id} className={`feature-bubble ${expanded.has(f.id) ? 'is-expanded' : ''} ${f.restricted ? 'is-restricted' : ''} ${dragging === f.id ? 'is-dragging' : ''}`} onDragStart={event => event.preventDefault()}>
      <Reference reference={`entry:${f.entry.id}`} kind={f.entry.kind} entry={{ ...f.entry, id: f.id, name: f.name, entries: Array.isArray(f.body) ? f.body : [f.body] }} sourceLabel={`${f.origin} · ${format(f.entry.source)}`} className="feature-caption" aria-expanded={expanded.has(f.id)} aria-label={`${inline ? '状态' : expanded.has(f.id) ? '折叠' : '展开'}${f.name}`} onClick={() => toggle(f.id)} onPointerDown={event => {
        ...
        cancel.current = pointerDrag(event, { title: f.name, subtitle: f.origin, outside: hit => !hit || !panel.current?.closest('.sheet-cell,.status-strip')?.contains(hit),
          ...
          finish: (_point, hit) => {
            ...
            if (valid && box) return { x: box.x, y: box.y };
          }
        });
      }} onKeyDown={event => { if (event.key === 'Delete') { event.preventDefault(); edit(draft => removeSelection(draft, f.id)); return; } if (!event.altKey || !['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown'].includes(event.key)) return; event.preventDefault(); const direction = ['ArrowLeft', 'ArrowUp'].includes(event.key) ? -1 : 1; const target = ordered[ordered.findIndex(item => item.id === f.id) + direction]; if (target) { const next = reordered(f.id, target.id, direction > 0); if (next) saveOrder(next); } }}>
        {expanded.has(f.id) ? <strong><em>{f.name.replace(/[。.]$/, '')}。</em></strong> : f.name}
      </Reference>{inline && conditionVisual(f.entry) === 'exhaustion' && <input className="exhaustion-level" aria-label="力竭层数" type="number" min="1" max="6" value={c.selections.find(s => s.id === f.id)?.level || 1} onChange={event => edit(draft => { const row = draft.selections.find(s => s.id === f.id); if (row) row.level = Math.max(1, Math.min(6, Math.trunc(Number(event.target.value) || 1))); })}/>} {expanded.has(f.id) && <div className="feature-prose rules-prose"><ContentBoundary key={f.id}><Entries value={f.body} onLink={onLink}/>{f.restricted && <p className="inline-warning">来源未启用</p>}</ContentBoundary></div>}
    </article>)}</div></section>)}
