...
export function Inline({ text, onLink }: { text: string; onLink?: LinkHandler }) {
  ...
  while ((match = regex.exec(text))) {
    ...
    if (['spell', 'item', 'class', 'race', 'feat', 'condition', 'skill', 'sense', 'variantrule', 'action', 'language', 'optfeature', 'background'].includes(tag)) parts.push(<Reference key={match.index} reference={body} kind={tag} onClick={() => onLink?.(body, tag)}>{label}</Reference>);
