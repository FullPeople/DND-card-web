...
export class ContentBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  render() { return this.state.failed ? <p className="inline-warning">这条资料的正文结构暂不支持。可以展开下方“数据与追溯”核对，或前往原站查阅。</p> : this.props.children; }
}
...
export function Inline({ text, onLink }: { text: string; onLink?: LinkHandler }) {
  ...
  while ((match = regex.exec(text))) {
    ...
    if (['spell', 'item', 'class', 'race', 'feat', 'condition', 'skill', 'sense', 'variantrule', 'action', 'language', 'optfeature', 'background'].includes(tag)) parts.push(<button key={match.index} className="inline-reference" onClick={() => onLink?.(body, tag)}>{label}</button>);
...
export function Entries({ value, onLink, depth = 0 }: { value: unknown; onLink?: LinkHandler; depth?: number }): ReactNode {
  ...
  if (typeof v.type === 'string' && v.type.startsWith('ref')) { const ref = v.classFeature || v.subclassFeature || v.optionalfeature; return typeof ref === 'string' ? <p><button className="inline-reference" onClick={() => onLink?.(ref, 'feature')}>{ref.split('|')[0]}</button></p> : null; }
