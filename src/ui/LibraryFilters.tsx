...
export function LibraryFilters({ tab, entries, filters, change, names }: { tab: LibraryTab; entries: Entry[]; filters: FacetSelection; change: (filters: FacetSelection) => void; names: Record<string, string> }) {
  ...
  const counts = useMemo(() => { const result = new Map<string, number>(); if (field) for (const e of entries) if (matchesFacets(e, filters, facets, field.key)) for (const v of new Set(field.values(e))) result.set(v, (result.get(v) || 0) + 1); return result; }, [entries, filters, facets, field]);
  return <><button className="filter-trigger" onClick={() => setOpen(true)}>筛选{count ? ` · ${count}` : ''}</button>{count > 0 && <button className="filter-reset" onClick={() => change({})}>清空</button>}
    {open && <dialog className="library-filter-dialog" ref={ref} onCancel={() => setOpen(false)} onClick={e => { if (e.target === ref.current) setOpen(false); }} aria-label="筛选规则资料">
      <header><strong>筛选资料</strong><span>{entries.filter(e => matchesFacets(e, filters, facets)).length} / {entries.length} 条</span><button aria-label="关闭筛选" onClick={() => setOpen(false)}>×</button></header>
      ...
      <div className="filter-body"><nav aria-label="筛选维度">{facets.map(f => <button key={f.key} className={f === field ? 'active' : ''} onClick={() => { setActive(f.key); setSearch(''); }}>{f.label}<small>{(filters[f.key]?.include.length || 0) + (filters[f.key]?.exclude.length || 0) || f.options.length}</small></button>)}</nav>
        ...
        </section></div><footer><button onClick={() => change({})}>重置全部</button><button className="primary" onClick={() => setOpen(false)}>完成 · 查看结果</button></footer>
    </dialog>}
  </>;
}
