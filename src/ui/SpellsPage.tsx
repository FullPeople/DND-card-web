...
import {ABILITIES,ABILITY_LABELS,type Selection} from '../core/model';
...
import {liftSpellTile} from './spellFlight';
...
export function SpellsPage({c,edit,browse,add,entries=[],inspect}:PageProps){
 ...
 function move(row:Selection,prepare:boolean,source?:HTMLElement,index?:number){
  const land=source?liftSpellTile(source):undefined;let changed=false,resultId=row.id;
    ...
    pointerDrag(e,{appearance:'source',title:row.entry.name,cancel:clear,
     ...
     finish:(_,hit)=>{
      ...
      if(canPrepare&&inPrepared(hit)){
       ...
       return {resolve:()=>preparedRoot.current?.querySelector<HTMLElement>(`[data-spell-id="${CSS.escape(id)}"]`)||source};
      }
      if(preparedTile&&!inPrepared(hit)){
       edit(draft=>{setPreparedSpell(draft,row.id,false);});
       return {resolve:()=>libraryRoot.current?.querySelector<HTMLElement>(`[data-spell-id="${CSS.escape(row.id)}"]`)||null};
      }
      if(editing&&stored&&!inLibrary(hit)){edit(draft=>removeSelection(draft,row.id));return {removed:true};}
     }
    });
   }}><SpellFrame/><span className="stock-name">{row.entry.name}</span><span className="spell-stock-level" aria-label={level===0?'戏法':`${level}环`} title={level===0?'戏法':`${level}环法术`}>{level}</span></Reference>;
 }
 return <>{editing&&<div className="spell-settings"><div className="segmented"><button aria-pressed={!settings.modeOverride} onClick={()=>change(s=>{s.modeOverride=false;})}>跟随职业</button>{(['known','prepared'] as const).map(mode=><button key={mode} aria-pressed={!!settings.modeOverride&&settings.mode===mode} onClick={()=>change(s=>{s.mode=mode;s.modeOverride=true;})}>{mode==='known'?'学习法术制':'预备法术制'}</button>)}</div><label>施法属性<select aria-label="施法属性" value={settings.ability} onChange={e=>change(s=>{s.ability=e.target.value as typeof s.ability;})}>{ABILITIES.map(a=><option key={a} value={a}>{ABILITY_LABELS[a]}</option>)}</select></label>{settings.mode==='prepared'&&<label>预备上限调整<NumberInput aria-label="预备上限调整" type="number" min="-100" max="100" value={settings.capacityAdjustment||0} onChange={e=>change(s=>{s.capacityAdjustment=Math.max(-100,Math.min(100,Math.trunc(Number(e.target.value)||0)));})}/></label>}</div>}
 ...
 <SheetCell label={fullList&&settings.mode==='prepared'?'职业法术表':'已学法术'} className="spell-library spell-stock-cell" dropKinds={['spell']} onReceive={add}><div ref={libraryRoot} className="spell-library-groups">{[...new Set(library.map(s=>Number(s.entry.raw.level)||0))].sort((a,b)=>a-b).map(level=><section key={level} className="spell-library-level"><h4>{level===0?'戏法':`${level}环`}</h4><div className="spell-stock-grid">{library.filter(s=>(Number(s.entry.raw.level)||0)===level).map(s=>tile(s))}</div></section>)}</div><button className="feature-browse" onClick={()=>browse('spell')}>＋ 查阅法术</button></SheetCell>
 {menu&&createPortal(<div className="stock-menu-shade spell-menu-shade" onPointerDown={e=>{if(e.target===e.currentTarget){e.preventDefault();setMenu(undefined);}}} onContextMenu={e=>{e.preventDefault();setMenu(undefined);}}><div role="menu" aria-label="法术操作" className="stock-menu spell-context-menu" style={{left:menu.x,top:menu.y}}><strong>{menu.row.entry.name}</strong>{settings.mode==='prepared'&&spellUsesPreparation(c,menu.row.entry)&&<button role="menuitem" onClick={()=>{move(menu.row,!prepared.includes(menu.row.id));setMenu(undefined);}}>{prepared.includes(menu.row.id)?'取消预备':'预备法术'}</button>}<button role="menuitem" onClick={()=>{inspect(menu.row.entry);setMenu(undefined);}}>在 Wiki 中查看</button>{editing&&c.selections.some(s=>s.id===menu.row.id)&&<button role="menuitem" onClick={()=>{edit(draft=>removeSelection(draft,menu.row.id));setMenu(undefined);}}>从角色卡移除</button>}</div></div>,document.body)}</>;
}
