...
import { Entries, Inline, RollLabelContext } from './Entries';
...
export function MonsterPortrait(_props:{entry:Entry}){return null;}
export function MonsterDocument({entry,onLink}:{entry:Entry;onLink:(reference:string,kind?:string)=>void}){
 ...
 function block(value:any,index:number){
  ...
  return <RollLabelContext.Provider value={String(value.name)} key={index}><section className="monster-trait"><p><strong><em><Inline text={String(value.name).replace(/[。.]$/,'')} onLink={onLink}/>{value.ENG_name&&value.ENG_name!==value.name&&<> <Inline text={String(value.ENG_name).replace(/[。.]$/,'')} onLink={onLink}/></>}。</em></strong>{typeof first==='string'&&<> <Inline text={first} onLink={onLink}/></>}</p><Entries value={typeof first==='string'?parts.slice(1):parts} onLink={onLink}/></section></RollLabelContext.Provider>;
 }
 ...
 <div className="monster-abilities" aria-label="怪物属性与豁免">{([['str','int'],['dex','wis'],['con','cha']] as Ability[][]).map((group,i)=><div className="monster-ability-group" key={i}><div className="monster-ability-labels"><span/><span/><span>调整</span><span>豁免</span></div>{group.map(a=><div className="monster-ability-row" key={a}><strong>{ABILITY_LABELS[a]}</strong><span>{r[a]??'—'}</span><button className="rollable-stat" disabled={!inWorkbench||r[a]==null} onClick={()=>composeRoll(`1d20${signed(mod(a))}`,`${ABILITY_LABELS[a]}检定`)}>{r[a]==null?'—':signed(mod(a))}</button><button className="rollable-stat" disabled={!inWorkbench||r[a]==null} onClick={()=>composeRoll(`1d20${signed(Number(r.save?.[a]??mod(a)))}`,`${ABILITY_LABELS[a]}豁免`)}>{r.save?.[a]??(r[a]==null?'—':signed(mod(a)))}</button></div>)}</div>)}</div>
 <div className="monster-defenses">{r.skill&&<p><strong>技能 </strong>{Object.entries(r.skill).map(([key,v])=><button key={key} className="rollable-stat" disabled={!inWorkbench||!Number.isFinite(Number(v))} onClick={()=>composeRoll(`1d20${signed(Number(v))}`,`${skills[key]||key}`)}>{skills[key]||key} {String(v)}</button>)}</p>}{fields.map(([label,value])=><p key={label}><strong>{label}</strong> <Inline text={value} onLink={onLink}/></p>)}{cr!=null&&<p><strong>CR</strong> {cr}（XP {(r.cr?.xp??xpByCR[String(cr)])?.toLocaleString()||'—'}{pb!=null?`; PB ${signed(pb)}`:r.pbNote?`; PB ${r.pbNote}`:''}）</p>}</div>
