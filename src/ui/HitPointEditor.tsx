...
export function HitPointEditor({c,edit}:{c:Character;edit:(fn:(c:Character)=>void)=>void}){
 const mode=c.baseHp>0?'manual':c.hpProgression?.mode||'average',d=evaluate(c),rows=hitPointLevels(c,d.modifiers.con);
 ...
 {mode==='manual'?<label>基础生命值上限<NumberInput aria-label="基础生命值上限" type="number" min="1" max="99999" value={c.baseHp} onChange={e=>edit(c=>{c.baseHp=Math.max(1,Number(e.target.value)||1);})}/></label>:<><p>体质调整值 {d.modifiers.con>=0?'+':''}{d.modifiers.con}；首个角色等级取满骰，其他等级可填写实际骰值。未填写时取固定平均值。</p><div className="hp-levels">{rows.map(r=><label key={`${r.id}:${r.level}`}><span>{r.name} {r.level}级 · d{r.faces}</span>{mode==='rolled'&&!r.first?<NumberInput aria-label={`${r.name}${r.level}级生命骰结果`} type="number" min="1" max={r.faces} value={r.die} onChange={e=>edit(c=>{const progression=c.hpProgression||={mode:'rolled',rolls:{}};const rolls=progression.rolls[r.id]||=[];rolls[r.level-1]=Math.max(1,Math.min(r.faces,Number(e.target.value)||1));})}/>:<b>{r.die}</b>}<span>＋体质＝{r.hp}</span></label>)}</div></>}
 <p>当前生命值上限：{d.maxHp}（含卡面调整 {c.sheetBonuses?.hp||0}）</p></section>;
}
