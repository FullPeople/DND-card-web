...
export function PersonalEntries({c,edit}:Props){
 ...
 function load(row?:Selection){
  setEditing(row?.id);setType(row?.entry.raw._customType||row?.entry.kind||'feature');setName(row?.entry.name||'');setBody(plainText(row?.entry.entries));
 ...
 function save(){try{
  const raw=JSON.parse(advanced),kind=CUSTOM_TYPES[type].kind;
  ...
  const entry=createCustomEntry({id:current?.entry.id,type,name,body,raw,revision:String(Number(current?.entry.revision||0)+1)});delete entry.raw._workbenchCustom;
 ...
 {type==='class'&&<div className="custom-fields"><label>等级<NumberInput aria-label="自定义职业等级" type="number" min="1" max="20" value={level} onChange={e=>setLevel(Math.max(1,Math.min(20,Math.trunc(Number(e.target.value)||1))))}/></label><label>生命骰<select value={faces} onChange={e=>setFaces(Number(e.target.value))}>{[4,6,8,10,12,20].map(n=><option key={n} value={n}>d{n}</option>)}</select></label></div>}
 ...
 {type==='spell'&&<label>环阶<select aria-label="自定义法术环阶" value={spellLevel} onChange={e=>setSpellLevel(Number(e.target.value))}>{Array.from({length:10},(_,n)=><option key={n} value={n}>{n?`${n}环`:'戏法'}</option>)}</select></label>}
