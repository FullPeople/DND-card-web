...
export function WorkbenchMonster({target,raw,online,onLink,editing:editingMode,setEditing:changeEditing}:{target:Target;raw:any;online:boolean;onLink:(reference:string,kind?:string)=>void;editing?:boolean;setEditing?:(value:boolean)=>void}){
 const [page,setPage]=useState<SheetPage>('主要'),[localEditing,setLocalEditing]=useState(false),[saving,setSaving]=useState(false);
 const editing=editingMode??localEditing,setEditing=changeEditing||setLocalEditing;
 ...
 {page==='背包'?<WorkbenchInventory id={`monster:${target.itemId}`}/>:<><Vitals stats={target.stats} input={input} locked={target.statsLocked!==false} lockLabel={target.statsLocked!==false?'解锁怪物生命条':'锁定怪物生命条'} disabled={!online} lock={target.role==='GM'?()=>void workbenchRequest('statsLock',{locked:target.statsLocked===false}).catch(setError):undefined}/><SheetCell label="怪物资料" className="monster-paper-content">{editing&&!disabled?<MonsterEditor key={target.key} value={raw||{name:target.name}} busy={saving} save={async data=>{setSaving(true);try{await workbenchRequest('monsterSave',{data,expected:raw});}finally{setSaving(false);}}}/>:raw&&<MonsterDocument entry={entry} onLink={onLink}/>}</SheetCell></>}
