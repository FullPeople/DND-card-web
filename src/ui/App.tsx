...
export default function App() {
  ...
  const c=useMemo(()=>storedCharacter?(inWorkbench&&wb.shared?{...storedCharacter,edition:roomRules!.edition,profile:roomRules!.profile,rulePacks:roomRules!.packs}:domesticCompact?withSiteSources(storedCharacter,workspace?.siteSources,activePacks):storedCharacter):undefined,[storedCharacter,roomRules,workspace?.siteSources,activePacks]);
  ...
  function createLocalCharacter(edition:Edition='2024'){const next=newCharacter(edition);return domesticCompact?(workspaceRef.current?.siteSources?withSiteSources(next,workspaceRef.current.siteSources,workspaceRef.current.packs):startAllSources(next,defaultSources)):next;}
  useEffect(()=>{
  ...
  function persist(next: Workspace) {
    if(domesticCompact)next=ensureSiteSources(next);
  ...
  function acceptWorkspace(value: Workspace) {
    ...
    if(domesticCompact)value=ensureSiteSources(value);
  ...
  useEffect(() => {
    ...
    const initialize = async (canWrite: boolean) => {
      ...
      try {
        ...
        if (value) {acceptWorkspace(value);if(domesticCompact&&!value.siteSources&&canWrite)persist(workspaceRef.current!);} else { const first = createLocalCharacter(); const initial: Workspace = { schemaVersion: 1, characters: [first], activeId: first.id, packs: [] }; if (canWrite) persist(initial); else acceptWorkspace(initial); }
  ...
  const edit: Edit = (action, key) => {
    ...
    const effective=inWorkbench&&roomRules?{...character,edition:roomRules.edition,profile:roomRules.profile,rulePacks:roomRules.packs}:domesticCompact?withSiteSources(character,current.siteSources,current.packs):character;
  ...
  async function editSources(action:(draft:Character)=>void,sourceMode?:'full'|'short'|'both'){
    if(!domesticCompact)return editRules(action,sourceMode);
  ...
  async function importFile(mode: string, file?: File, text?: string) {
    ...
    try {
      ...
      if (mode === 'pack') {
        ...
        if(inWorkbench||domesticCompact){if(!await editSources(draft=>{draft.rulePacks=[...activePacks.filter(p=>p.id!==pack.id),pack];draft.profile.enabledSources=[...new Set([...draft.profile.enabledSources,pack.id])];}))return;}else persist({ ...w, packs: [...w.packs.filter(p => p.id !== pack.id), pack], characters: w.characters.map(x => x.id === w.activeId ? { ...x, profile: { ...x.profile, enabledSources: [...new Set([...x.profile.enabledSources, pack.id])] } } : x) });
