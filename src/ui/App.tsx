...
export default function App() {
  ...
  const edit: Edit = (action, key) => {
    ...
    const effective=inWorkbench&&roomRules?{...character,edition:roomRules.edition,profile:roomRules.profile,rulePacks:roomRules.packs}:character;
  ...
  async function changeCustom(entry:Entry,remove=false){
    ...
    else {const w=workspaceRef.current!;persist({...w,customEntries:next,characters:w.characters.map(row=>row.id===w.activeId?{...row,profile:{...row.profile,enabledSources:[...new Set([...row.profile.enabledSources,'CUSTOM'])]}}:row)});}
  ...
  return <KeywordPreview isExcluded={entry=>explicitlyExcluded(c,entry)} resolve={resolveReference} open={link} sheetPreview={entry=>library.preview(entry?readingTarget(entry):undefined)} sheetCommit={entry=>inspect(entry)}><EntryDragProvider editing={editing&&(!inWorkbench||!!wb.target?.write)} character={c} receive={entry => add(entry)}><div className="app-shell" data-workbench-page={inWorkbench?workbenchPage:undefined} onDragStart={event => event.preventDefault()}>
    ...
    <main className={`workspace ${wikiVisible||tableOpen?'':'wiki-hidden'}`}>
      ...
      {tableOpen?<><WorkspaceSplitter/><section className={`wiki-pane table-pane ${tab==='wiki'?'mobile-active':''}`} aria-label="三龙牌"><WorkbenchPanel panel="table" close={()=>setTableOpen(false)}/></section></>:wikiVisible&&<><WorkspaceSplitter/><section className={`wiki-pane ${tab === 'wiki' ? 'mobile-active' : ''}`} aria-label="规则资料"><div className="wiki-header"><div><span className="eyebrow">规则资料</span><span className="wiki-source">5etools 中文站</span></div><button title="重新检查上游资料" disabled={loading} onClick={() => load(true)}>{loading ? '加载中…' : '更新资料'}</button></div>
        ...
        <footer className="wiki-footer"><a href={inWorkbench?'./source.zip':'https://github.com/FullPeople/DND-card-web'} target="_blank" rel="noreferrer">源码 ↗</a><a href="https://github.com/FullPeople/DND-card-web/blob/main/LICENSE" target="_blank" rel="noreferrer">非商用共享许可 ↗</a></footer>
      </section></>}
    </main>
    ...
    {modal && <Dialog title={modal === 'characters' ? '角色簿' : modal === 'rules' ? '规则与扩展' : modal === 'export' ? '导入与导出' : modal === 'adjust' ? '数值依据与人工修正' : modal === 'resources' ? '法术位与资源记录' : modal === 'quickbar' ? '整理快捷栏' : '让角色卡带你完成选择'} close={() => { setModal(''); setImportError(''); setConfirmDelete(''); }}>
      ...
      {modal === 'rules' && <fieldset className="rules-fieldset" disabled={rulesBusy} aria-busy={rulesBusy}>{inWorkbench&&wb.role!=='GM'?<RoomRulesSummary character={c} entries={allEntries} scope={wb.shared?.scope}/>:<>
        ...
        <SourceSettings c={c} entries={allEntries} edit={editRules} readOnly={rulesReadonly} changeMode={sourceDisplay.setMode}/>
