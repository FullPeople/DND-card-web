...
function RequirementView({ r, c, edit, find, drop, active }: { r: Requirement; c: Character; edit: Edit; find: (r: Requirement) => void; drop: (e: DragEvent, r: Requirement) => void; active: boolean }) {
  ...
  return <div className={`requirement ${r.complete ? 'is-complete' : ''} ${active ? 'is-target' : ''}`} onDragOver={e => e.preventDefault()} onDrop={e => drop(e, r)} data-requirement={r.id}>
...
function Selected({ s, c, edit, inspect }: { s: Selection; c: Character; edit: Edit; inspect: (e: Entry) => void }) {
  ...
  return <div className={`selected-entry ${allowed ? '' : 'restricted'}`}>
    <div className="selected-title"><button className="text-link" onClick={() => inspect(s.entry)}>{s.entry.name}</button><button className="remove" aria-label={`移除${s.entry.name}`} title="移除，可撤销" onClick={() => edit(d => { d.selections = d.selections.filter(x => x.id !== s.id); })}>×</button></div>
...
export default function App() {
  ...
  function create(edition: Edition, copy = false) {
    ...
    persist({ ...workspace, characters: [...workspace.characters, next], activeId: next.id }); setTargetId(''); setModal(''); setTab('sheet');
  }
  const requirements = (sections: string[]) => d?.requirements.filter(r => sections.includes(r.section) && (showAllRequirements || !r.complete || r.options)).map(r => <RequirementView key={r.id} r={r} c={c!} edit={edit} find={find} drop={drop} active={targetId === r.id}/>);
  const selections = (kinds: Kind[]) => c?.selections.filter(s => kinds.includes(s.entry.kind)).map(s => <Selected key={s.id} s={s} c={c} edit={edit} inspect={inspect}/>);
      ...
      <section className={`sheet-pane ${tab === 'sheet' ? 'mobile-active' : ''}`} aria-label="角色卡工作区">
        ...
        <div className="sheet-scroll"><div className="paper" style={{ width: `${zoom}%`, minWidth: `${Math.round(590 * zoom / 100)}px` }}>
          ...
          <div className="identity-grid"><div className="identity-main">
            ...
            <Box label="职业与等级"><div onDragOver={e => e.preventDefault()} onDrop={e => drop(e, d.requirements.find(r => r.id === 'base:class'), ['class', 'subclass'])}>{selections(['class', 'subclass'])}{requirements(['class', 'subclass'])}{c.selections.some(s => s.entry.kind === 'class') && addButton('subclass')}</div></Box>
            <div className="identity-bottom"><Box label="种族"><div onDragOver={e => e.preventDefault()} onDrop={e => drop(e, d.requirements.find(r => r.id === 'base:race'), ['race'])}>{selections(['race'])}{requirements(['race'])}</div></Box><Box label="性别"><input aria-label="性别" value={c.identity.gender} onChange={e => edit(d => { d.identity.gender = e.target.value; }, 'gender')}/></Box><Box label="背景"><div onDragOver={e => e.preventDefault()} onDrop={e => drop(e, d.requirements.find(r => r.id === 'base:background'), ['background'])}>{selections(['background'])}{requirements(['background'])}</div></Box><Box label="阵营"><input aria-label="阵营" value={c.identity.alignment} onChange={e => edit(d => { d.identity.alignment = e.target.value; }, 'alignment')}/></Box></div>
          ...
          <div className="paper-main"><div className="stats-columns"><div className="stats-left">
            ...
            <Box label="装备与物品" className="equipment-box"><div onDragOver={e => e.preventDefault()} onDrop={e => drop(e, undefined, ['item'])}>{selections(['item'])}{requirements(['item'])}{addButton('item')}<p className="sheet-note">穿戴的护甲和盾牌参与护甲计算。物品效果请核对正文。</p></div></Box>
          </div></div>
