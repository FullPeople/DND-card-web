...
import { ABILITIES, ABILITY_LABELS, KIND_LABELS, SKILLS, newCharacter, selectionAllowed, uid, type Character, type Edition, type Entry, type Kind, type Requirement, type Selection } from '../core/model';
...
function RequirementView({ r, c, edit, find, active }: { r: Requirement; c: Character; edit: Edit; find: (r: Requirement) => void; active: boolean }) {
  ...
  return <SheetCell label={r.label} className={`requirement-field ${active ? 'is-target' : ''}`} missing={!r.complete} requirementId={r.id} onFill={() => find(r)} dropKinds={r.kind ? [r.kind] : undefined} dropRequirement={r}>
    <small className="requirement-origin">{r.origin} · {r.selected.length}/{r.count}</small>
    ...
    })}/>{r.optionLabels?.[v] || (r.id.endsWith('ability-mode') ? `方案 ${Number(v) + 1}` : choiceLabel(v))}</label>)}</div> : r.complete && <button className="choose-button" onClick={() => find(r)}>查看可选内容 →</button>}
  </SheetCell>;
}
...
export default function App() {
  ...
  const edit: Edit = (action, key) => {
    ...
    const draft = structuredClone(character); action(draft); if (draft.quickbar) draft.quickbar = draft.quickbar.filter(id => draft.selections.some(s => s.id === id)); draft.updatedAt = new Date().toISOString(); draft.revision++;
  ...
  function browse(kind: Kind) { setTargetId(''); setKind(kind); setQuery(''); setSource(''); setEditionFilter('character'); setDetail(undefined); setTab('wiki'); }
  ...
  function add(entry: Entry, requirement: Requirement | null | undefined = target, pin = false) {
    if (!c) return;
    if (pin) { const existing = c.selections.find(s => s.entry.id === entry.id); if (existing && selectionAllowed(c, entry)) { edit(draft => { draft.quickbar = [...new Set([...(draft.quickbar || []), existing.id])].slice(0, 100); }); setNotice(`已固定「${entry.name}」到快捷栏`); return; } }
    if (requirement?.options) { setNotice('这个位置需要直接勾选选项。'); return; }
    ...
    edit(draft => { const selectionId = uid(); draft.selections.push({ id: selectionId, entry: structuredClone(entry), quantity: 1, level: 1, equipped: false, ...(requirement && !requirement.id.startsWith('base:') ? { requirementId: requirement.id } : {}) }); if (pin) draft.quickbar = [...(draft.quickbar || []), selectionId].slice(0, 100); });
    setNotice(`已填入「${entry.name}」`);
  }
  function drop(e: DragEvent, requirement?: Requirement, expected?: Kind[], pin = false) {
    ...
    if (expected && !expected.includes(entry.kind)) { setNotice(`这个位置接收${expected.map(k => KIND_LABELS[k]).join('、')}。`); return; } add(entry, requirement || null, pin);
  }
  ...
  const renderRequirement = (r: Requirement) => <RequirementView key={r.id} r={r} c={c!} edit={edit} find={find} active={targetId === r.id}/>;
  const requirements = (sections: string[]) => d?.requirements.filter(r => sections.includes(r.section) && (showAllRequirements || !r.complete || r.options)).map(renderRequirement);
  const renderSelection = (s: Selection) => <Selected key={s.id} s={s} c={c!} edit={edit} inspect={inspect} chooseSubclass={s.entry.kind === 'class' ? () => { const r = d?.requirements.find(r => r.kind === 'subclass' && r.parentClass?.name === s.entry.name && r.parentClass?.source === s.entry.source); if (r) find(r); else browse('subclass'); } : undefined}/>;
  ...
  const blocked = target?.options ? '请先完成上方的直接选择，或退出当前填写。' : detail ? candidateReason(c, detail, target) : '';
  return <KeywordPreview resolve={resolveReference} open={link}><DragContext.Provider value={{ entry: dragged, character: c, drop, end: endDrag }}><div className="app-shell">
    ...
    <main className="workspace">
      <section className={`sheet-pane ${tab === 'sheet' ? 'mobile-active' : ''}`} aria-label="角色卡工作区">
        ...
        <PaperFrame page={sheetPage} changePage={page => { setSheetPage(page); setTab('sheet'); }}>
          ...
          {sheetPage === '主要' ? <><Overview c={c} d={d} edit={edit} find={find} browse={browse} inspect={inspect} renderSelection={renderSelection} renderRequirement={renderRequirement} openResources={() => setModal('resources')} openQuickbar={() => setModal('quickbar')} pinDrop={e => drop(e, undefined, ['item', 'spell', 'feature', 'feat', 'rule'], true)}/>
      ...
      <section className={`wiki-pane ${tab === 'wiki' ? 'mobile-active' : ''}`} aria-label="规则资料"><div className="wiki-header"><div><span className="eyebrow">规则资料</span><span className="wiki-source">5etools 中文站</span></div><button title="重新检查上游资料" disabled={loading} onClick={() => load(true)}>{loading ? '加载中…' : '更新资料'}</button></div>
        ...
        {target?.options && <section className="wiki-choice-editor" aria-label="当前填写选项"><h3>{target.label} <small>{target.selected.length}/{target.count}</small></h3><div className="choice-options">{target.options.map(value => <label key={value}><input type={target.count === 1 ? 'radio' : 'checkbox'} name={`wiki:${target.id}`} checked={target.selected.includes(value)} onChange={() => edit(draft => { const values = draft.answers[target.id] || []; draft.answers[target.id] = target.count === 1 ? [value] : values.includes(value) ? values.filter(v => v !== value) : values.length < target.count ? [...values, value] : values; })}/>{target.optionLabels?.[value] || choiceLabel(value)}</label>)}</div></section>}
    ...
    {modal && <Dialog title={modal === 'characters' ? '角色簿' : modal === 'rules' ? '规则与扩展' : modal === 'export' ? '导入与导出' : modal === 'adjust' ? '数值依据与人工修正' : modal === 'resources' ? '法术位与资源记录' : modal === 'quickbar' ? '整理快捷栏' : '让角色卡带你完成选择'} close={() => { setModal(''); setImportError(''); setConfirmDelete(''); }}>
      ...
      {modal === 'help' && <div className="help-copy"><p>从左侧角色卡开始。流动的虚线外框表示这个格子还有待填写的内容，点击格子，右侧会定位对应资料或选项。</p><ol><li>在角色簿中选择 2014 或 2024；在规则设置中勾选允许使用的书籍。</li><li>先填基础属性，再挑选职业、种族和背景。等级变化会产生新的填写要求。</li><li>阅读右侧正文，把条目拖入对应位置；也可以点“填入当前要求”。技能与属性增长直接在卡内选择。</li><li>特殊效果保留正文并提示核对。与 DM 协商的例外可记录在条目中。</li><li>随时导出完整备份。审卡 HTML 可离线发给 DM，也可打印为 PDF。</li></ol><p>这不是完整的战斗模拟器。基础属性、熟练、护甲、生命值和部分选择可以计算；施法准备、复杂前提、特性联动和休息恢复仍需人工核对。没有资料的规则不会被假装实现。</p><p>账号同步、Excel 适配和 C++ 桌面客户端属于后续阶段。</p><a href="https://github.com/FullPeople/DND-card-web" target="_blank" rel="noreferrer">源代码与问题反馈 ↗</a></div>}
    </Dialog>}
