...
export function saveVital(entity:Target|CardChoice,field:string,expression:string|number,label:string,remember=true,expected?:number):Promise<number>{
 ...
 const send=async(previous?:any)=>{const wb=getWorkbench(),live=cardId?wb.cards.find(c=>c.id===cardId):wb.monsters.find(c=>c.itemId===itemId);const selected=wb.target?.itemId===entity.itemId?wb.target:undefined;
  ...
  const parsed=typeof expression==='number'?{value:expression,relative:false}:numericExpression(expression,before||0);
  const next=Math.floor(parsed.value),delta=next-(before||0),normalized=parsed.relative?`${delta>=0?'+':''}${delta}`:next;
  const result=await workbenchRequest('stats',{key:undefined,itemId,patch:{[field]:normalized},expected:{[field]:before}});if(!result.snapshot)return {...result,committedValue:before};const after=result.snapshot.state.stats[field];
